  ...
  return <div className="information-bar">
    ...
    {galleryItems && <section className="information-section information-gallery" aria-labelledby="gallery-thumbnails-heading">
      ...
      {galleryItems.length > 0
        ? <nav className="gallery-thumbnails" aria-label="其他画廊展示" style={{ '--thumbnail-columns': thumbnailColumns, '--thumbnail-rows': thumbnailRows } as CSSProperties}>{galleryItems.slice(0, thumbnailColumns * thumbnailRows).map((item) => <Link key={item.id} to={`/gallery/${item.id}`} aria-label={`查看画廊展示：${item.title}`} aria-current={item.id === currentGalleryId ? 'page' : undefined} style={{ aspectRatio: String(thumbnailAspectRatio(item)) }}><GalleryCropImage src={item.url} alt="" loading="lazy" focus={item.thumbnailFocus} aspectRatio={item.thumbnailAspectRatio} cropPositioning={item.cropPositioning} width={item.width} height={item.height} fallbackAspectRatio={1} /></Link>)}</nav>
